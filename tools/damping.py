...
def IonNeutral_Damping(E, medium_props, nu_n = 0, theta = 0) : 
    ...
    # Fully ionized phases 
    if (X == 1.0) : 
        return dict(wr = k*VAi*np.cos(theta), wi = 0., VA = VAi)
    
    # Partially ionized phases 
    if (X < 1.0) : 
        chi = (mn/mi)*(X**(-1.)-1.)
...
#    print "nn = ",nn," cm^-3"
    ...
    # Fully ionized phases 
    if (X == 1.0) : 
        wr = k*VAi*np.cos(theta)
        wi = 0. 
        
        return dict(wr=wr, wi = -wi, VA = VAi)
...
def damping_lazarian_nopos(E, medium_props) : 
    ...
    # Only for partially ionized phases 
    if (X < 1.0) : 
