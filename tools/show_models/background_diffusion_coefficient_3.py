...
def Duu_Alfven_Slab_Linear_Undamped(mu, E, medium_props, particles_props, kmin = 1e-20, q = 1.5, I = 1e-4) : 
    """
...
def Kappa_zz(E, medium_props, particles_props, kmin = 1e-20, q = 5./3, I = 1e6) : 
    ...
    for jj in range(1, len(mu)-1) : 
        ...
        k_zz += dmumu*(1 - mu[jj]**2)**2/Duu_Alfven_Slab_Linear_Undamped(mu[jj], E, medium_props, particles_props, kmin = kmin, q = q, I = I)
...
for ii in range(len(E)) : 
    K_zz_1[ii] = Kappa_zz(E[ii], ism.WNM, {"m":cst.mp}, kmin = 1e-20, q = 5./3, I = 1e6)
    K_zz_2[ii] = Kappa_zz(E[ii], ism.DiM, {"m":cst.mp}, kmin = 1e-20, q = 5./3, I = 1e6)
...
# for ii in range(len(E)) : 
#     m = cst.mp
#     gamma = 1 + (E[ii] /(m*cst.c**2))
#     v = cst.c*np.sqrt(1 - (1/(E[ii]/(m*cst.c**2) + 1))**2)
#     p = gamma*m*v 
#     Omega0 = cst.e*medium_props.get("B")/(m*cst.c)
#     Omega  = Omega0/gamma 
...
#     for jj in range(1, len(mu) - 1) : 
#         dmumu = 0.5*(mu[jj+1] - mu[jj-1])
#         k_zz += dmumu*(1 - mu[jj]**2)**2/Duu_Alfven_Slab_Linear_Undamped(mu[jj], medium_props, particles_props, kmin = 1e-20, q = 5/3., I = Itot)
        
    
#     K_zz[ii] = v**2/8*k_zz


# plt.loglog(E/cst.GeV, K_zz)

# mu = np.linspace(-1, 1, 1000)
# D_1  = np.zeros(len(mu))
# k_zz_1 = 0.
# for ii in range(1, len(mu)-1) : 
    
#     D_1[ii] = Duu_Alfven_Slab_Linear_Undamped(mu[ii], medium_props, particles_props, kmin = 1e-20, q = 5./3, I = Itot)
    
#     dmumu = 0.5*(mu[ii+1] - mu[ii-1])
#     if (D_1[ii] != 0) : 
#         k_zz_1 += (v**2/8)*(1 - mu[ii]**2)**2/(D_1[ii])*dmumu
    
    
# plt.semilogy(mu, D_1, c="blue")



# print ("K_zz_1 = ",k_zz_1," cm^2/s")
