...
k_space = cst.e*phases[0].get("B")/E

# xlim   = [Emin/cst.GeV, Emax/cst.GeV]
xlim   = [k_space[0]*cst.pc, k_space[-1]*cst.pc]
xlims  = [xlim]
...
for pi in range(len(phases)) : 
    ...
    for e in range(len(E)) : 
        ...
        kp = cst.e*phases[pi].get("B")/Ep
        km = cst.e*phases[pi].get("B")/Em
    ...
    ax.loglog(k_space*cst.pc, wR_Alfven, c="blue")
    ax.loglog(k_space*cst.pc, -wI_Alfven, c="black")
    ...
    ax.axvspan(km*cst.pc, kp*cst.pc, alpha=0.5, color='grey')
    ...
    ax.set_title(Name[pi], x = 0.9, y = 0.9)
    
    # if (pos_1[pi] == 3 and pos_2[pi] == 0) : 
    ax.plot([],[], c = "blue", label = "$\\omega_R^A$")
    ax.plot([],[], c = "black", label = "$-\\Gamma_I^A$")
...
# plt.tight_layout()
plt.savefig("in_damping.png", bbox_inches='tight',pad_inches=-0., dpi=500)