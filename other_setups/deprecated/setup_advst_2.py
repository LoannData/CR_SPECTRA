...
import namelist_advst_2 as nml
...
for ei in range(len(E)) : 
    ...
    for xi in range(len(X)) : 
        ...
        # if (X[xi] >= x_center) : 
        #VA[ei][xi] = 0.5*cst.pc/cst.yr*(10.*(X[xi]-X[0])/(2000*cst.pc))
        
        Pcr[ei][xi] = spec(E[ei], 2.7)
        Pe[ei][xi] = spec(E[ei], 0.5)
        # Pcr[ei][xi] = door(E[ei], 1e2*cst.GeV, 1e3*cst.GeV, 1.)
        
        D[ei] = (5*cst.pc)**2/cst.yr
        Db[ei][xi] = (4*cst.c)/(3*np.pi)*(E[ei]/(cst.e*B[x_center_index])) 
        Ip[ei][xi] = Db[ei][xi]/D[ei][xi]*0.5
        Im[ei][xi] = Db[ei][xi]/D[ei][xi]*0.5
