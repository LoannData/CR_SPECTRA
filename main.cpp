...
int main()
{
    ...
    */
    ...
    std::string general_parameters = "./params/gparams.dat";
    std::string sDIMENSION = search(general_parameters, "DIMENSION");  int DIMENSION = stoi(sDIMENSION); 
    std::string snx = search(general_parameters, "NX"); int nx = stoi(snx);
    int ny; if (DIMENSION > 1) {std::string sny = search(general_parameters, "NY"); int ny = stoi(sny);}
    std::string sne = search(general_parameters, "NE"); int ne = stoi(sne);
    std::string GRID = search(general_parameters, "GRID");


    std::string specific_parameters = "./params/specparams.dat";
    std::string sni = search(specific_parameters,"ni");        double ni = stod(sni);
    std::string sX  = search(specific_parameters,"X");         double Xi = stod(sX); 
    std::string smn = search(specific_parameters,"mn");        double m_neutral = stod(smn); 
    std::string sT   = search(specific_parameters,"T");        double T  = stod(sT);
    /*std::string sEsn = search(specific_parameters,"Esn");      double Esn = stod(sEsn); 
    std::string sMej = search(specific_parameters,"Mej");      double Mej = stod(sMej); 
    std::string sxi_n = search(specific_parameters, "xi_n");   double xi_n = stod(sxi_n);
    std::string sphi_c = search(specific_parameters,"phi_c");  double phi_c = stod(sphi_c);
    std::string sbeta = search(specific_parameters, "beta");   double beta = stod(sbeta); */
        ...
        for (xi = 0; xi<NX; xi++)
        {
            ...
            for (ei = 0; ei<NE; ei++)
            {
                ...
                for (lxi = 0; lxi < 5; lxi++)
                {
                    ...
                    //Qcrs_temp = Qcrs(E_temp, time, box_X[2], ni, Xi, m_neutral, T);
                    ...
                    //if (xi == 1 and ei == 1){cout<<Pcr_old[xi][ei]<<"-"<<Pcr_new_temp<<endl;}
